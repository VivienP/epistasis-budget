...
Each interaction ε ~ N(ε_hat, σ²). Measuring a variant reveals a ΔG that is a fixed linear combination
in each loop the variant belongs to, so the posterior-variance update is standard linear-Gaussian
conditioning. Total uncertainty is non-increasing in the measured set and info_gain is submodular —
this is what licenses greedy selection (docs/RESEARCH_EPISTASIS.md#6).
"""
...
class EpistasisFactorGraph:
    ...
    def info_gain(self, measured: frozenset[Variant], candidate: Variant) -> float:
        """Reduction in total uncertainty from additionally measuring ``candidate`` (≥ 0, submodular)."""
        raise NotImplementedError("Week 1 — see docs/ROADMAP.md")
